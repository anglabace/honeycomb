...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    /**
     * 函数名：查询函数-根据用户名、密码查询用户信息 - getUser（）
     * 功能描述： 根据账户名、密码获取当前用户的相关信息
     * 输入参数：<按照参数定义顺序>
     *
     * @param username String类型的用户名
    ...
    public User getUser(String username, String password) {
        return userMapper.selectOne(new QueryWrapper<User>().lambda().eq(User::getUsername, username).eq(User::getPassword, password));
    }
